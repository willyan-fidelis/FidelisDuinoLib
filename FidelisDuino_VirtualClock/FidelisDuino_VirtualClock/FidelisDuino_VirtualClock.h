	...
	namespace VirtualClock{
		...
		private:
			...
			void Fill() {
				...
				// imprime os valores na porta serial para confer�ncia
				for (int i = 0; i < 50; i++) {
			...
			String zeller_name_friendly(int year, int month, int day)
			{
				...
				case 2:
					...
					break;
				case 3:
					return "Terca-feira";//Serial.println("Ter�a-feira");
					break;
				case 4:
				...
				case 6:
					...
					break;
				case 0:
					return "Sabado";//Serial.println("S�bado");
					break;
				}
		...
		public:
			...
			void GetTime(unsigned long& hrs, unsigned long& min, unsigned long& sec, unsigned long& weekday, unsigned long& year, unsigned long& month, unsigned long& day) {
				...
				month = _month;
				day = _day;
				//Forcamos pegar o dia sa semana conforme a data, pois � garantido realizando o calculo:
				...
				if (clock.ChangedToNewDay())
				{
					...
					for (size_t i = 0; i < 20; i++)
					{
						if (years[i].year == _year)
						{
							...
							{
								//Se entramos aqui � pq estamos no ultimo dia do mes:
								_day = 1;
								_month++;
